...
void sRGBColorEncoding::FromLinear(std::span<const Float> vin,
...
void sRGBColorEncoding::ToLinear(std::span<const uint8_t> vin,
...
void GammaColorEncoding::ToLinear(std::span<const uint8_t> vin,
                                  std::span<Float> vout) const {
    DCHECK_EQ(vin.size(), vout.size());
    for (size_t i = 0; i < vin.size(); ++i)
...
void GammaColorEncoding::FromLinear(std::span<const Float> vin,
                                    std::span<uint8_t> vout) const {
    DCHECK_EQ(vin.size(), vout.size());
    for (size_t i = 0; i < vin.size(); ++i)
        vout[i] =
...
DFPBRT_CONST Float SRGBToLinearLUT[256] = {
